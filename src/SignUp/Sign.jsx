...
export default function Sign() {
    ...
    const submitDetails = (e) => {
        ...
        if (!validateUserName(username)) {
            toast.error("Name should be Atleast 4 Characters and No digits or Special Characters allowed");
            ...
            axios.post(baseurl + '/addNewUser', userdata)
                ...
                .catch((err) => {
                    setSubmission(false)
                    if (err.response) {
                        // Check if the response object and its data are defined
                        if (err.response.data && err.response.data.message) {
                            toast.error(err.response.data.message);  // Display the error message
                        } else {
                            toast.error("An error occurred. Please try again.");
                        }
                    } else {
                        // In case there is no response (e.g., network error)
                                    ...
                                    <Col md={8}>
                                        <div id="card-input-signup">
                                            ...
                                            <button className="btn btn-outline-dark" id="otp-btn" onClick={getOtp} disabled={genOtpDisabled}>
                                                {genOtpDisabled ? 'Generating OTP' : 'Generate OTP'}
                                            </button>
